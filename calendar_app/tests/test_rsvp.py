...
class GuestRSVPTest(RSVPTestMixin, TestCase):
    ...
    def test_guest_timestamp_added_on_create(self):
        self.client.post(self.url, {"guest_name": "Guest", "status": "coming"})
        self.event.refresh_from_db()
        self.assertIn("timestamp", self.event.guests[0])

    def test_guest_timestamp_updated_on_status_change(self):
        self.client.post(self.url, {"guest_name": "Guest", "status": "coming"})
        self.event.refresh_from_db()
        old_ts = self.event.guests[0]["timestamp"]

        resp = self.client.post(self.url, {"guest_name": "Guest", "status": "not_coming"})
        self.assertEqual(resp.status_code, 200)
        self.event.refresh_from_db()
        self.assertNotEqual(self.event.guests[0]["timestamp"], old_ts)

    def test_guest_timestamp_unchanged_on_same_status(self):
        self.client.post(self.url, {"guest_name": "Guest", "status": "coming"})
        self.event.refresh_from_db()
        old_ts = self.event.guests[0]["timestamp"]
        ...
        self.client.post(self.url, {"guest_name": "Guest", "status": "coming"})
        self.event.refresh_from_db()
        self.assertEqual(self.event.guests[0]["timestamp"], old_ts)


class GuestWaitlistTest(RSVPTestMixin, TestCase):
    def setUp(self):
        self.client = Client()
        self.event = self._create_event(max_participants=2)
    ...
    def test_guest_counted_in_total_coming(self):
        ...
        self.client.post(self.guest_url, {"guest_name": "Guest1", "status": "coming"})
        self.client.post(self.guest_url, {"guest_name": "Guest2", "status": "coming"})

        self.event.refresh_from_db()
        guests = get_guests(self.event, self.event.date)
