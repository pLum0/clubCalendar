...
class GetAllowedNtfyHostsTest(TestCase):
    def test_ntfy_sh_always_included(self):
        hosts = _get_allowed_ntfy_hosts()
        self.assertIn("ntfy.sh", hosts)
    ...
    def test_configured_hosts_are_reachable(self):
        hosts = _get_allowed_ntfy_hosts()
        for host in hosts:
    ...
    def _probe_host(self, host):
        topic = f"test_{secrets.token_hex(8)}"
        payload = secrets.token_hex(16)
        base_url = f"https://{host}/{topic}"
        timeout = httpx.Timeout(5.0, read=15.0)

        try:
            client = httpx.Client(http2=True, timeout=timeout)
        except Exception as e:
            self.skipTest(f"could not create client for {host}: {e}")

        try:
            with client.stream("GET", f"{base_url}/json") as r:
                lines = r.iter_lines()
                next(lines)

                try:
                    httpx.post(base_url, content=payload, headers={"Title": "test"}, timeout=5)
                except (httpx.HTTPError, OSError) as e:
                    self.skipTest(f"could not publish to {host}: {e}")
                ...
                for line in lines:
                    ...
                    if data.get("message") == payload:
                        found = True
                        break

                if not found:
                    raise AssertionError(f"{host}: notification did not arrive within timeout")
        except (httpx.HTTPError, OSError) as e:
            self.skipTest(f"could not connect to {host}: {e}")
        finally:
            client.close()
...
class NotifyWaitlistUserTest(TestCase):
    ...
    def test_sends_notification_to_enabled_user(self, mock_async):
        ...
        event = Event.objects.create(title="Test", date=date(2025, 6, 15), start_time=time(10, 0))
        rsvp = RSVP.objects.create(event=event, occurrence_date=event.date, user=user, status="coming")
...
class NotifyRsvpsEventChangeTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user_no_ntfy = CalendarUser.objects.create(name="Bob", team=self.team, ntfy_enabled=False, language="en")
        self.event = Event.objects.create(title="Test Event", date=date(2025, 6, 15), start_time=time(10, 0))

    @patch("calendar_app.notifications.send_notification_async")
    def test_cancelled_notifies_enabled_users(self, mock_async):
        RSVP.objects.create(
            event=self.event, occurrence_date=self.event.date, user=self.user_with_ntfy, status="coming"
        )
        RSVP.objects.create(event=self.event, occurrence_date=self.event.date, user=self.user_no_ntfy, status="coming")
    ...
    def test_notice_notification(self, mock_async):
        RSVP.objects.create(event=self.event, occurrence_date=self.event.date, user=self.user_with_ntfy, status="maybe")
    ...
    def test_time_changed_notification(self, mock_async):
        ...
        notify_rsvps_event_change(self.event, None, "time_changed", start_time=time(14, 0), end_time=time(16, 0))
        mock_async.assert_called_once()
    ...
    def test_not_coming_users_not_notified(self, mock_async):
        RSVP.objects.create(
            event=self.event, occurrence_date=self.event.date, user=self.user_with_ntfy, status="not_coming"
        )
    ...
    def test_occurrence_date_filter(self, mock_async):
        ...
        RSVP.objects.create(
            event=self.event, occurrence_date=date(2025, 6, 22), user=self.user_with_ntfy, status="coming"
        )
    ...
    def test_language_per_user(self, mock_async):
        ...
        RSVP.objects.create(event=self.event, occurrence_date=self.event.date, user=user_de, status="coming")
        RSVP.objects.create(
            event=self.event, occurrence_date=self.event.date, user=self.user_with_ntfy, status="coming"
        )
        notify_rsvps_event_change(self.event, None, "cancelled")
