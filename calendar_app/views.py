...
def _parse_guest_timestamp(guest):
    ts_str = guest.get("timestamp", "")
    if ts_str:
...
def _get_merged_status_list(rsvps_by_status, guests_by_status, status_key):
    entries = []
    for r in rsvps_by_status.get(status_key, []):
        entries.append(
            {
                "type": "user",
                "name": r.user.name,
                "team_name": r.user.team.name,
                "comment": r.comment,
                "timestamp": r.status_updated_at,
                "rsvp_obj": r,
            }
        )
    for g in guests_by_status.get(status_key, []):
        entries.append(
            {
                "type": "guest",
                "name": g["name"],
                "team_name": None,
                "comment": None,
                "timestamp": _parse_guest_timestamp(g),
            }
        )
...
def _check_waitlist_promotion(event, occurrence_date):
    ...
    coming_rsvps = list(
        RSVP.objects.filter(event=event, occurrence_date=occurrence_date, status="coming")
        .select_related("user")
    ...
    for r in coming_rsvps:
        entries.append(("rsvp", r.status_updated_at, r))
    for g in guests:
        if g.get("status") == "coming":
            entries.append(("guest", _parse_guest_timestamp(g), g))
...
def get_event_occurrences(event, start_date, end_date, calendar_user=None, rsvp_data=None):
    ...
    if event.recurrence:
        ...
        occurrence_details = {od.occurrence_date: od for od in event.occurrence_details.all()}
        for occ_date in recurrence_dates:
            ...
            if rsvp_data is not None:
                ...
                if calendar_user:
                    try:
                        user_rsvp = RSVP.objects.get(event=event, occurrence_date=occ_date_only, user=calendar_user)
        ...
        if start_date <= event.date <= end_date:
            if rsvp_data is not None:
                ...
                if calendar_user:
                    try:
                        user_rsvp = RSVP.objects.get(event=event, occurrence_date=event.date, user=calendar_user)
                        user_rsvp_status = user_rsvp.status
                    except RSVP.DoesNotExist:
                        pass
...
def _prefetch_rsvps(events, calendar_user):
    ...
    rsvp_qs = RSVP.objects.filter(event_id__in=event_ids, status="coming").values_list("event_id", "occurrence_date")
    ...
    user_rsvps = {}
    if calendar_user:
        user_rsvp_qs = RSVP.objects.filter(event_id__in=event_ids, user=calendar_user).values_list(
            "event_id", "occurrence_date", "status"
        )
    ...
    for event in events:
        result[event.id] = {
            ...
            "user_by_date": {date: status for (eid, date), status in user_rsvps.items() if eid == event.id},
        }
    return result
...
def calendar_view(request):
    ...
    for event in events_list:
        occurrences = get_event_occurrences(
            event, first_day, last_day, calendar_user, rsvp_data=rsvp_data.get(event.id)
        )
    ...
    if not calendar_user:
        all_users_data = list(CalendarUser.objects.select_related("team").values("id", "name", "team_id", "team__name"))
...
def event_detail(request, event_id):
    ...
    all_rsvps = list(RSVP.objects.filter(event=event, occurrence_date=occurrence_date).select_related("user__team"))
    ...
    team_players = CalendarUser.objects.filter(team__in=event.tags.all()).select_related("team").order_by("name")
    if calendar_user:
    ...
    merged_lists = {}
    total_counts = {}
    for status_key in ["coming", "maybe", "not_coming"]:
        merged_lists[status_key] = _get_merged_status_list(rsvps_by_status, guests_by_status, status_key)
        total_counts[status_key] = len(merged_lists[status_key])
...
def rsvp(request, event_id):
    ...
    rsvp_obj, created = RSVP.objects.update_or_create(
        event=event, occurrence_date=occurrence_date, user=calendar_user, defaults=defaults
    )
...
def guest_rsvp(request, event_id):
    ...
    if existing:
        ...
        guests.append({"name": name, "status": status, "timestamp": now})
...
def upcoming_events(request):
    ...
    all_occurrences = []
    for event in events_list:
        occurrences = get_event_occurrences(event, today, end_date, calendar_user, rsvp_data=rsvp_data.get(event.id))
...
def update_occurrence(request, event_id):
    ...
    details, created = OccurrenceDetails.objects.update_or_create(
        event=event, occurrence_date=occurrence_date, defaults=defaults
    )
